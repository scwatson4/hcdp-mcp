# HCDP MCP Server — Evolved Toolset v2.0
**Verified against:** `hcdp_api.yaml` (HCDP OpenAPI spec V1)  
**Language:** Python (FastMCP)  
**Server name:** `hcdp_mcp`  
**Date:** 2026-03-17

---

## Corrections from v1.0 (API Compatibility Fixes)

| Severity | Tool | Issue Fixed |
|---|---|---|
| 🔴 CRITICAL | `get_timeseries_data` + `_fetch_single_island_year` | Path was `/timeseries` → corrected to `/raster/timeseries` |
| 🔴 CRITICAL | `get_timeseries_data` | Params `start_date`/`end_date` → corrected to `start`/`end` per spec |
| 🔴 CRITICAL | `get_timeseries_data` + `_fetch_single_island_year` | Removed invalid `fill` param (only exists in `/stations`, not `/raster/timeseries`) |
| 🔴 CRITICAL | `_fetch_single_island_year` | Was passing `var_id` to raster endpoint → corrected to `datatype` |
| 🔴 CRITICAL | `get_timeseries_data` + `_fetch_single_island_year` | Added missing required `extent` param |
| 🔴 CRITICAL | `get_station_latest` | `/mesonet/db/stationMonitor` has no `station_ids` param → removed; fixed response parsing to match actual schema (`24hr_latest` keyed dict) |
| 🔴 CRITICAL | `export_mesonet_csv_via_email` | Was using GET (`hcdp_get`) → corrected to POST with JSON body; fixed `station_ids`/`var_ids` to arrays |
| 🟠 MAJOR | All raster tools | `ClimateDatatype` enum conflated Mesonet var_ids with raster datatypes → split into `RasterDatatype` and `MesonetDatatype` enums |
| 🟡 MINOR | `get_mesonet_data` | Limit cap raised from 500 to 10000 (API default); max set to 1,000,000 per spec |
| 🟡 MINOR | `get_mesonet_data` | `join_metadata` changed from `bool` to `Optional[str]` per spec (truthy string, not boolean) |
| 🟡 MINOR | `get_mesonet_variables` | Removed invalid `location` param (not in spec for this endpoint) |
| 🟡 MINOR | `get_mesonet_data` | Added missing optional params: `flags`, `reverse`, `local_tz`, `row_mode` |

---

## Shared Infrastructure

Paste at the **top** of your server file, above all tool definitions.
...
# ── Constants ─────────────────────────────────────────────────────────────────
HCDP_API_BASE   = "https://api.hcdp.ikewai.org"
...
**Endpoint:** `GET /mesonet/db/measurements`  
**Changes from v1:** Limit raised to 10,000 (API default); added `flags`, `reverse`, `local_tz`, `row_mode`; `join_metadata` changed to `Optional[str]`.
...
**Endpoint:** `GET /mesonet/db/stations`  
**Changes from v1:** `status` filter is now client-side (not a server param per spec).

```python
class GetMesonetStationsArgs(BaseModel):
    model_config = ConfigDict(extra="forbid")

    location: str = Field(default="hawaii", description="Network location. Options: 'hawaii', 'american_samoa'.")
...
**Endpoint:** `GET /mesonet/db/variables`  
**Changes from v1:** Removed invalid `location` param (not in spec for this endpoint).
...
## Tool 4 — `get_island_climate_history` ⭐ PRIMARY EVOLVED TOOL

**Endpoint:** `GET /raster/timeseries` (called internally per region, in parallel)  
**Changes from v1:** Fixed path (`/raster/timeseries`), param names (`start`/`end`), removed `fill`, fixed `datatype` vs `var_id`, added `extent`.
...
async def get_island_climate_history(params: GetIslandClimateHistoryArgs) -> str:
    ...
    A single call replaces what previously required 20 sequential calls (4 islands × 5 years).
...
No changes needed — routes directly to `get_island_climate_history`.
...
**Endpoint:** `GET /raster/timeseries`  
**Changes from v1:** Fixed path, param names (`start`/`end`), removed `fill`, added `extent`, fixed datatype to use `RasterDatatype` enum, added `aggregation` and `timescale`.
    ...
    try:
        ...
        if params.response_format == ResponseFormat.MARKDOWN:
            lines = [
                ...
                f"**Period:** {params.start} → {params.end} | **Extent:** {params.extent.value} | **Aggregation:** {params.period.value}",
...
**Endpoint:** `GET /mesonet/db/stationMonitor`  
**Changes from v1:** Removed invalid `station_ids` param (not in spec for this endpoint).
Fixed response parsing to match actual `MesonetStationMonitorData` schema (keyed dict with `24hr_latest`).
...
async def get_station_latest(params: GetStationLatestArgs) -> str:
    ...
    try:
        ...
        if not isinstance(data, dict):
            return json.dumps({"error": "Unexpected response format from stationMonitor.", "raw": str(data)[:200]})
        ...
        if not stations_out and params.station_id:
            return json.dumps({
                "error": f"Station {params.station_id} not found in stationMonitor response. "
...
No API compatibility changes — geographic computation is entirely client-side using `/mesonet/db/stations`.
...
No API compatibility changes — built entirely on `get_island_climate_history`.
...
class GetIslandComparisonArgs(BaseModel):
    ...
    aggregation: Optional[TempAggregation] = Field(default=None, description="Temperature aggregation.")
    response_format: ResponseFormat = Field(default=ResponseFormat.JSON)
...
async def get_island_comparison(params: GetIslandComparisonArgs) -> str:
    ...
    try:
        ...
        result = {"year": params.year, "datatype": params.datatype.value, "ranked": ranked_data, "summary": summary}
...
No API compatibility changes — built on `get_island_climate_history`. Updated to use `RasterDatatype`.

```python
class GetDroughtIndexArgs(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")

    islands: Union[IslandName, List[IslandName]] = Field(..., description="Island or list of islands.")
...
async def get_drought_index(params: GetDroughtIndexArgs) -> str:
    ...
    try:
        island_list = [params.islands] if isinstance(params.islands, (str, IslandName)) else list(params.islands)
        fetch_params = GetIslandClimateHistoryArgs(
            islands=island_list,
        ...
        for island_name in island_strs:
            ...
            for year in params.reference_years:
                ...
                if val is not None and stddev > 0:
                    ...
                    spi_by_year[str(year)] = {"value": val, "spi": spi, "interpretation": interpret_spi(spi)}
                elif val is not None:
                    spi_by_year[str(year)] = {"value": val, "spi": None, "interpretation": "Insufficient variance"}
        ...
        if params.response_format == ResponseFormat.MARKDOWN:
            ...
            for r in results:
                ...
                if "error" in r:
                    ...
                    continue
                lines.append(f"**Mean:** {r['reference_mean']} | **Stddev:** {r['reference_stddev']} | **Trend:** {r['trend']}")
                lines.append("\n| Year | Value | SPI | Interpretation |\n|---|---|---|---|")
                for yr, v in r["spi_by_year"].items():
                    lines.append(f"| {yr} | {v.get('value','—')} | {v.get('spi','—')} | {v.get('interpretation','')} |")
                lines.append("")
            return "\n".join(lines)
...
**Endpoint:** `POST /mesonet/db/measurements/email`  
**Changes from v1:** Changed from GET to POST (critical fix); request body is JSON with `email` + `data` object; `station_ids` and `var_ids` are arrays not strings; uses `hcdp_post`.
    ...
    Request body format per YAML spec:
      {
        ...
        "data": {
          "location": "hawaii",
          "station_ids": ["0145", "0141"],   ← array, not comma-separated
          "var_ids": ["RH_1_Min", "Tair_2_Max"],  ← array
...
| # | Tool Name | Endpoint | Method | Status |
|---|---|---|---|---|
| 1 | `get_mesonet_data` | `/mesonet/db/measurements` | GET | ✅ Evolved |
| 2 | `get_mesonet_stations` | `/mesonet/db/stations` | GET | ✅ Keep |
| 3 | `get_mesonet_variables` | `/mesonet/db/variables` | GET | ✅ Fixed |
| 4 | `get_island_climate_history` | `/raster/timeseries` (parallel) | GET | ⭐ New/Fixed |
| 5 | `get_island_history_summary` | routes to #4 | — | ⚠️ Deprecated alias |
| 6 | `get_timeseries_data` | `/raster/timeseries` | GET | ✅ Fixed |
| 7 | `get_station_latest` | `/mesonet/db/stationMonitor` | GET | ✅ Fixed |
| 8 | `get_nearest_stations` | `/mesonet/db/stations` | GET | ✅ Keep |
| 9 | `get_island_comparison` | routes to #4 | — | ✅ New |
| 10 | `get_drought_index` | routes to #4 | — | ✅ New |
| 11 | `export_mesonet_csv_via_email` | `/mesonet/db/measurements/email` | **POST** | ✅ Fixed |

**Removed:** `get_island_current_summary`, `compare_current_vs_historical`, `get_city_current_weather`, `get_station_data`
